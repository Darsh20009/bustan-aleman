...
-   **Quran Reader**: Displays Quran text, translations, tafsir, and offers reciter selection with audio. Includes word-level highlights, notes, memorization progress tracking (range-based, mastery levels, review scheduling), and daily reading statistics. **Recitation system (التسميع)**: Uses Web Speech API for real-time Arabic speech recognition with sequential ayah tracking and word-by-word comparison. Detects iframe context and prompts user to open in standalone browser window when microphone access is blocked.
...
-   **AI Recitation System (Smart Tasmi')**: Complete mobile-first Quran recitation system at `/quran/recitation`. Page-based (604 pages matching Mushaf). Features: real-time word-level green/red highlighting with pending state for unrecited words, bottom error bar showing tajweed/memorization/order mistakes, auto-page-navigation when reciting from a different page, session scoring with per-ayah breakdown, AI-powered repeated error reporting to responsible sheikh via `POST /api/ai/report-recitation-errors`, floating mic button optimized for mobile touch. Uses Web Speech API (`ar-SA`), Levenshtein similarity with 0.7 threshold, and `compareWordsDetailed()` with `isFinal` flag for progressive vs final evaluation.
...
-   **Kirox Video Sessions**: Full Kirox QMeet API integration (`server/kiroxService.ts`). When teacher enables a session, Kirox meeting is created and URL stored in `sessionAccess.zoomLink`. Both teacher and student dashboards open Kirox URL directly (fallback to internal room if no Kirox URL). Idempotent: reuses existing Kirox URL if session already has one. WebSocket notifies students with Kirox URL when session is enabled.
