...
// Live Session Rooms (in-platform video/audio rooms)
export const liveRooms = bustanSchema.table("live_rooms", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  sessionId: varchar("session_id"),
  sheikhId: varchar("sheikh_id").notNull(),
...
export const roomParticipants = bustanSchema.table("room_participants", {
  ...
  isVideoEnabled: boolean("is_video_enabled").default(false),
});
...
export type SelectRoomParticipant = typeof roomParticipants.$inferSelect;
...
export const liveAnnotations = bustanSchema.table("live_annotations", {
  ...
  surahNumber: integer("surah_number").notNull(),
  ayahNumber: integer("ayah_number").notNull(),
  annotationType: varchar("annotation_type").notNull(),
...
export type SelectLiveAnnotation = typeof liveAnnotations.$inferSelect;
